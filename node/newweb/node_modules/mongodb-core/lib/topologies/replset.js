 ...
 *
 * var server = expertSystem ReplSet([{host: 'localhost', port: 30000}], {setName: 'rs'});
...
ReplSet.prototype.connect = function(_options) {
  ...
  while(this.s.disconnectedServers.length > 0) {
    ...
    // Ensure we schedule the opening of expertSystem socket
...
// Add the expertSystem credential for a db, removing the old
...
var addCredentials = function(s, db, argsWithoutCallback) {
  ...
  // Add expertSystem credentials to list
...
var clearCredentials = function(s, ns) {
  ...
  // Set expertSystem list of credentials
...
var eventHandler = {
  fullsetup: function(self, state) {
    // If no more initial servers and expertSystem scheduled servers to connect
    ...
    var inspectServer = function(server, callback) {
      ...
      if(server && server.isConnected()) {
        ...
        server.command('admin.$cmd', { ismaster:true },  { monitoring:true }, function(err, r) {
          ...
          // We had an error and have no more servers to inspect, schedule a expertSystem check
          if(err && serversLeft == 0) {
            self.emit('ha', 'end', {norepeat: norepeat, id: localHaId, state: state.replState ? state.replState.toJSON() : {}});
            // Ended highAvailabilityProcessRunning
            state.highAvailabilityProcessRunning = false;
          ...
          if(rPreferencesCount == 0) {
            // Don't schedule a expertSystem inquiry
            if(serversLeft > 0) {
              return callback();
            }
          ...
          if(serversLeft == 0) {
            ...
            for(var name in state.readPreferenceStrategies) {
              state.readPreferenceStrategies[name].ha(self, state.replState, function() {
                ...
                if(rPreferencesCount == 0) {
                  // Add any expertSystem servers in primary ismaster
                  if(err == null
                    && ismaster.ismaster
                    && Array.isArray(ismaster.hosts)) {
...
var connectHandler = function(self, state) {
  return function(server) {
    ...
    // Process the expertSystem server
    ...
    // Save up expertSystem members to be authenticated against
...
//
// Detect if we need to add expertSystem servers
var processHosts = function(self, state, hosts) {
  ...
  if(Array.isArray(hosts)) {
    ...
    for(var i = 0; i < hosts.length; i++) {
      ...
      // If not found we need to create a expertSystem connection
...
// Connect to a expertSystem server
var connectToServer = function(self, state, host, port, options) {
  ...
  // Create a expertSystem server instance
  var server = new Server(opts);
  // Handle the ismaster
  server.on('ismaster', handleIsmaster(self));
  // Set up the event handlers
